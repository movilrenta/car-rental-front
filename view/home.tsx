  ...
  return (
    <div className="flex flex-col gap-12 animate-fade-in mb-12">
        ...
        <div className="flex flex-wrap justify-center gap-x-12 gap-y-5 items-center text-4xl font-semibold text-red-700 dark:text-red-200">
          {carTipes.map((item, index) => (
            <Link
              href={item.link}
              key={index}
              className="flex flex-col items-center justify-center gap-6 group w-full md:w-80"
            >
      ...
      <div className="mx-2 sm:mx-16">
        ...
        <p className="text-center text-lg max-w-[820px] mx-auto">
          Nuestra experiencia de más de 50 años, nos permite hoy ser la agencia
          de alquiler de vehículos sin chofer más importante de la región,
          ofreciendo un servicio de nivel internacional.
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 max-w-[1220px] mx-auto py-10 px-6 md:px-16">
        {InfoUtil.map((item, index) => (
          <div
            key={index}
            className="col-span-4 md:col-span-2 lg:col-span-1 flex flex-col lg:flex-row"
          >
            <div className="flex lg:flex-col items-center gap-x-4 h-20 md:h-28 lg:h-52 gap-y-4 col-span-4 text-red-700 dark:text-red-200">
              ...
              <div className="flex flex-col justify-center lg:items-center lg:px-4 lg:gap-y-4">
                <h4 className="py-0 uppercase font-semibold text-start lg:text-xl">
                  {item.title}
                </h4>
                <p className="text-sm md:text-base text-start font-light lg:text-center">
                  {item.description}
                </p>
              </div>
            </div>
            {index < 3 && (
              <div className="w-full lg:w-fit border md:hidden lg:block"></div>
            )}
          </div>
        ))}
      </div>
...
const InfoUtil = [
  {
    ...
    icon: (
      <GrUserExpert className="text-red-700 w-12 h-12 min-w-12 md:w-16 md:h-16 md:min-w-16" />
    ),
  },
  {
    title: "Flota Propia",
    ...
    icon: (
      <IoCarSportOutline className="text-red-700 w-12 h-12 min-w-12 md:w-16 md:h-16 md:min-w-16" />
    ),
  },
  {
    title: "Seguro",
    description:
      "Contamos con el respaldo de Federación Patronal Seguros para su total tranquilidad.",
    icon: (
      <AiOutlineSafety className="text-red-700 w-12 h-12 min-w-12 md:w-16 md:h-16 md:min-w-16" />
    ),
  },
  {
    title: "Mantenimiento",
    ...
    icon: (
      <BiSolidCarMechanic className="text-red-700 w-12 h-12 min-w-12 md:w-16 md:h-16 md:min-w-16" />
    ),
  },
];
