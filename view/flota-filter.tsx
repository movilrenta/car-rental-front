    ...
    <div className="max-w-7xl mx-auto mt-6 px-4 grid grid-cols-12 gap-6">
      <div className="col-span-12 flex flex-wrap justify-center gap-2 w-full max-h-fit">
        <Button
          variant="outline"
          className={`${
            filter === "car" ? "bg-red-700 text-white" : "group"
          } hover:text-white hover:bg-red-500 duration-200 w-[120px] min-w-[120px]`}
        ...
        >
          ...
          {filter === "car" ? (
            ...
            <ArrowLeft className="hidden group-hover:inline stroke-2" />
          )}
        </Button>
        <Button
          variant="outline"
          className={`${
            filter === "utility" ? "bg-red-700 text-white" : "group"
          } hover:text-white hover:bg-red-500 duration-200 w-[120px] min-w-[120px]`}
          onClick={() => handleFilter("utility")}
        >
          {t("filter.utility")}
          {filter === "utility" ? (
            <IoMdCloseCircle className="" />
        ...
        <Button
          variant="outline"
          className={`${
            filter === "van" ? "bg-red-700 text-white" : "group"
          } hover:text-white hover:bg-red-500 duration-200 w-[120px] min-w-[120px]`}
      ...
      {Object.entries(agrupedCars).map(([name, Cars]: any, index) => {
        ...
        return firstCar
          ? (filter === firstCar.vehicle_type || filter === "all") && (
              <div
                className={`col-span-12 md:col-span-4 duration-200`}
                key={firstCar.id}
              >
